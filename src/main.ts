...
const elements = {
  ...
  copyButton: queryRequired<HTMLButtonElement>("#copyButton"),
...
function startRankingSession(photos: PhotoItem[]): AppState {
...
function advanceToNextComparison(nextState = state): AppState {
...
function createExportData(): object {
  return {
    session: {
...
function loadState(): AppState {
  ...
  try {
    ...
    if (!Array.isArray(parsed.photos)) return emptyState();
    return {
